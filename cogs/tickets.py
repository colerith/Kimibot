...
class TicketActionView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ 接收并开始一审", style=discord.ButtonStyle.success, custom_id="ticket_claim_review1")
    async def claim_review1(self, button: discord.ui.Button, interaction: discord.Interaction):
    ...
    async def review2(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        try:
            ...
            # 移动频道并改名
            new_name = f"二审中-{info.get('工单ID', '未知')}-{info.get('创建者', '未知')}-{info.get('ReviewerName', '未知')}"
