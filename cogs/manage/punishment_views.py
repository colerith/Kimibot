...
class ManagementControlView(ui.View):
    ...
    async def cb_user(self, select, i): await i.response.defer(); self.selected_user=select.values[0]; self.selected_user_id=self.selected_user.id; await self.refresh_view(i)
    @ui.select(placeholder="🔨 选择动作...", row=1, custom_id="sel_act", options=[
        discord.SelectOption(l,v,e) for l,v,e in [("警告 (Warn)","warn","⚠️"),("禁言 (Mute)","mute","🤐"),("踢出 (Kick)","kick","🚀"),("封禁 (Ban)","ban","🚫"),("解除禁言","unmute","🎤"),("解除封禁","unban","🔓")]])
    async def cb_act(self, select, i): await i.response.defer(); self.action_type=select.values[0]; await self.refresh_view(i)
    @ui.button(l="ID搜人",s=2,r=2,e="🔍",cid="btn_id")
    async def cb_id(self,_,i): await i.response.send_modal(IDInputModal(self))
    @ui.button(l="追加证据",s=2,r=2,e="📎",cid="btn_ev")
    async def cb_ev(self,_,i): await i.response.send_modal(EvidenceAppendModal(self))
    @ui.button(l="理由/时长",s=1,r=2,e="📝",cid="btn_reason")
    async def cb_rsn(self,_,i): await i.response.send_modal(ReasonInputModal(self))
    ...
    async def cb_exec(self, _, interaction: discord.Interaction):
        ...
        member = None
        try: member = guild.get_member(tid) or await guild.fetch_member(tid)
        except discord.NotFound: pass
        ...
        try:
            ...
            # ... (此段落无逻辑修改，仅为排版)
            if act=="warn": msg_act,color="进行警告",0xFFAA00; \
                (await member.send(embed=discord.Embed(title=f"⚠️ {guild.name} 社区警告",description=f"**理由:** {self.reason}",color=color).set_image(url=f"attachment://{(f:=[await a.to_file(spoiler=True) for a in self.attachments])[0].filename}" if self.attachments else None), files=f) if member else None)
            elif act=="mute": msg_act,color=f"禁言 ({self.duration_str})",0xFF5555; secs=parse_duration(self.duration_str); \
                (await member.timeout(discord.utils.utcnow()+datetime.timedelta(seconds=secs),reason=self.reason) if member and secs>0 else (_ for _ in ()).throw(ValueError("用户不存在或时长无效")))
            elif act=="kick": msg_act,color="踢出",0xFF0000; (await member.kick(reason=self.reason) if member else (_ for _ in ()).throw(ValueError("用户不存在")))
            elif act=="ban": msg_act,color="封禁",0x000000; await guild.ban(discord.Object(id=tid), reason=self.reason, delete_message_days=0)
            elif act=="unmute": msg_act,color="解禁",0x55FF55; (await member.timeout(None, reason=self.reason) if member else (_ for _ in ()).throw(ValueError("用户不存在")))
            elif act=="unban": msg_act,color="解封",0x00AAFF; await guild.unban(discord.Object(id=tid), reason=self.reason)
            ...
            if public_chan:
                ...
                if user_obj.display_avatar: p_embed.set_thumbnail(url=user_obj.display_avatar.url)
            ...
            if log_chan:
                ...
                if act == "mute": log_embed.add_field(name="时长", value=self.duration_str, inline=True)
